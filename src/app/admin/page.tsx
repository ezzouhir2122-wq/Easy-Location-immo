...
export default async function AdminPage() {
  ...
  const [{ count: clients }, { count: subscriptions }, { count: activeClients }, { count: pendingClients }, { data: recentClients }, { data: allClients }, { data: subscriptionRows }] = await Promise.all([
  ...
  const planPrices: Record<string, number> = { basic: 49, pro: 149, agency: 299 };
  const monthlyRevenue = (subscriptionRows ?? []).reduce((total, row) => total + (planPrices[row.plan] ?? 0), 0);
  const growth = Array.from({ length: 6 }, (_, index) => { const date = new Date(); date.setMonth(date.getMonth() - (5 - index)); const key = `${date.getFullYear()}-${date.getMonth()}`; return { label: date.toLocaleDateString("fr-FR", { month: "short" }), count: (allClients ?? []).filter((client) => { const created = new Date(client.created_at); return `${created.getFullYear()}-${created.getMonth()}` === key; }).length }; });
  ...
  return (
    <div className="admin-ocean">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Revenus mensuels" value={`${monthlyRevenue.toLocaleString("fr-FR")} DH`} accent="emerald" href="/admin/subscriptions" />
      </div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2"><Stat label="Clients en attente" value={pendingClients ?? 0} accent="orange" href="/admin/clients" /><div className="rounded-2xl border border-white/10 bg-white/[.06] p-5"><div className="flex items-center justify-between"><p className="text-xs text-slate-400">Croissance des clients</p><span className="text-xs text-cyan-300">6 mois</span></div><div className="mt-5 flex h-20 items-end gap-2">{growth.map((item) => <div key={item.label} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-md bg-gradient-to-t from-cyan-500 to-blue-400" style={{ height: `${Math.max((item.count / maxGrowth) * 100, item.count ? 12 : 3)}%` }} /><span className="text-[9px] text-slate-500">{item.label}</span></div>)}</div></div></div>
...
function Stat({ label, value, accent, href }: { label: string; value: string | number; accent: string; href: string }) {
  const colors: Record<string, string> = { cyan: "#67e8f9", blue: "#60a5fa", violet: "#c4b5fd", emerald: "#6ee7b7", orange: "#fdba74" };
